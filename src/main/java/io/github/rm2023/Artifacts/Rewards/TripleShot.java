...
public class TripleShot extends Passive {
    ...
    public String getDescription() {
        return "Fire three arrows in rapid sequence.";
    }

    @Override
    public ItemStack getRepresentationStack() {
        ItemStack item = new ItemStack(Material.TIPPED_ARROW, 3);
        ...
        return new ArrayList<Passive>() {
            {
                ...
                add((Passive) Main.plugin.rewardMap.get("PENTA_SHOT"));
            }
        };
    }

    @EventHandler(ignoreCancelled = true, priority = EventPriority.HIGHEST)
    public void TripleShotEvent(EntityShootBowEvent event) {
        if (validateWorld(event.getEntity().getWorld()) && enabledPlayers.contains(event.getEntity()) && event.getProjectile() instanceof Arrow) {
            ...
            for (int i = 1; i <= 2; i++) {
                Main.plugin.getServer().getScheduler().scheduleSyncDelayedTask(Main.plugin, () -> {
                    ...
                    String customData = getCustomData(event.getEntity());
                    if (getCustomData(event.getEntity()).contains("EXPLOSIVE_ARROWS") && !((Player) event.getEntity()).getInventory().removeItem(new ItemStack(Material.GUNPOWDER, 5)).isEmpty()) {
                        customData.replace("EXPLOSIVE_ARROWS", "");
                    }
                    if (getCustomData(event.getEntity()).contains("FIREWORK_ARROWS") && !((Player) event.getEntity()).getInventory().removeItem(new ItemStack(Material.GUNPOWDER, 1)).isEmpty()) {
